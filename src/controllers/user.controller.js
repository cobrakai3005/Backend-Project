...
if(existedUser){
    throw new ApiError(409, "User with email or username already exisists!")
}
...
const user = await User.create({fullName,avatar: avatar.url, email, password, coverImage: coverImage?.url || "", username: username.toLowerCase()});
...
return res.status(200).json(new ApiResponse(200, createdUser, "User registerd succesfully"))
});
...
const loggOutUser = wrapAsync(async(req, res)=>{
...
const updateAccountDetails = wrapAsync(async(req, res)=>{
...
return res.status(200).json(new ApiResponse(200, user, "Account details updated succesfull.y"))
...
const updateUserCoverImage = wrapAsync(async(req, res) =>{
 ...
 const coverImage = await uploadOnCloudinary(coverImageLocalPath)
 if(!coverImage.url){
   throw new ApiError(400, "Error while uploadin on cover image" )
 }
...
export {registerUser, loginUser, loggOutUser, refreshAccessToken, changeCurrentPassword, getCurrentUser, updateAccountDetails, updateUserAvatar, updateUserCoverImage}